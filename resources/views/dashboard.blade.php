...
@section('content')
    ...
    <div class="flex items-center justify-between">
        <div class="space-y-1">
            ...
            <h2 class="font-bold text-2xl text-blue-700">{{ Auth::user()->nama_lengkap ?? Auth::user()->name }} 👋</h2>
        </div>
        <div class="text-right">
            <p class="text-xs text-gray-500 uppercase tracking-wider font-bold">Tahun Ajaran</p>
            <p class="text-sm font-bold text-blue-700">{{ $tahunAktif->nama_semester ?? 'Belum diatur' }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-gutter">
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2 py-1 bg-blue-50 text-blue-700 text-[10px] font-bold rounded uppercase">Total</span>
            </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
            <div class="relative z-10">
                <p class="text-xs text-gray-500 uppercase font-bold tracking-wider">Presensi Hari Ini</p>
                <div class="flex items-center gap-4 mt-3">
                    <div class="text-center">
                        <p class="text-xl font-bold text-blue-600">{{ $hadirCount }}</p>
                        <p class="text-[10px] text-gray-500 uppercase">Hadir</p>
                    </div>
                    <div class="text-center">
                        <p class="text-xl font-bold text-amber-500">{{ $sakitCount }}</p>
                        <p class="text-[10px] text-gray-500 uppercase">Sakit</p>
                    </div>
                    <div class="text-center">
                        <p class="text-xl font-bold text-purple-500">{{ $izinCount }}</p>
                        <p class="text-[10px] text-gray-500 uppercase">Izin</p>
                    </div>
                    <div class="text-center">
                        <p class="text-xl font-bold text-red-500">{{ $alpaCount }}</p>
                        <p class="text-[10px] text-gray-500 uppercase">Alpa</p>
                    </div>
                </div>
            </div>
            <span
                class="material-symbols-outlined absolute -bottom-4 -right-4 text-gray-100 text-8xl opacity-50">monitoring</span>
        </div>
    </div>
                ...
                @foreach ($jadwalSepekan as $jadwal)
                    @if ($jadwal->hari !== $currentHari)
                        <div class="px-8 py-2 bg-gray-50 border-t border-gray-100 mt-2 first:mt-0 first:border-0">
                            <p class="text-xs font-bold text-blue-700 uppercase tracking-wider">{{ $jadwal->hari }}</p>
                        </div>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 px-4 md:px-8 py-4 hover:bg-gray-50 transition-colors">
                        ...
                        <a href="{{ route('presensi.show', $jadwal->kelas_id) }}?jadwal_id={{ $jadwal->id }}"
                            class="px-3 py-1.5 bg-blue-50 text-blue-700 text-xs font-bold rounded-lg hover:bg-blue-600 transition-colors text-center mt-2 md:mt-0 w-full md:w-auto">
                            Mulai Presensi
                        </a>
                    </div>
                @endforeach
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('kelas.index') }}" class="text-sm text-blue-700 font-medium hover:underline">Lihat Semua
                →</a>
        </div>
            ...
            @foreach ($kelasList as $kelas)
                <a href="{{ route('kelas.show', $kelas->id) }}"
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="font-bold text-blue-700">{{ $kelas->siswa_aktif_count }}</span>
                    </div>
                </a>
            @endforeach
        </div>
