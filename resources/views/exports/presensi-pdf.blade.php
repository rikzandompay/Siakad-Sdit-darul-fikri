...
<head>
    <meta charset="UTF-8">
    <title>Presensi {{ $kelas->nama_kelas }} - SDIT Darul Fikri</title>
    <style>
...
<body onload="window.print()">
    <div class="header">
        ...
        <h2>Rekap Presensi {{ $kelas->nama_kelas }}</h2>
        <p>Periode: {{ $rentangLabel }} ({{ \Carbon\Carbon::parse($dateRange['start'])->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($dateRange['end'])->format('d/m/Y') }})</p>
    </div>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:left">{{ $row['siswa']->nis }}</td>
                <td style="text-align:left"><strong>{{ $row['siswa']->nama_siswa }}</strong></td>
                <td>{{ $row['hadir'] }}</td>
                <td>{{ $row['sakit'] }}</td>
                <td>{{ $row['izin'] }}</td>
                <td>{{ $row['alpa'] }}</td>
